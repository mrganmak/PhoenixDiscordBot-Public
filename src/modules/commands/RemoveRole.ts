	...
	private async getRole(
		...
		if (userRoles.length > 1) {
			const selectMenuOptions: IPollQuestionSelectMenu = {
				type: 'selectMenu',
				content: 'Какую роль вы хотите снять?',
				answers: [],
			...
			targetMember.roles.remove(roleId).catch(console.error);
		} else if (userRoles.length == 1) {
			const hasRoleGeted = await targetMember.roles.remove(userRoles[0]).catch(console.error);
